    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        recycler.setAdapter(new FirebaseRecyclerAdapter<Post, PostViewHolder>(options) {
            ...
            protected void onBindViewHolder(final PostViewHolder viewHolder, int position, final Post post) {
                ...
                viewHolder.author_2.setText(post.author +" -> "+ post.content);
                ...
                if (post.mediaUrl != null) {
                    ...
                    } else if("video".equals(post.mediaType)) {
                        viewHolder.video.setVisibility(View.VISIBLE);
                        viewHolder.video.setVideoPath(post.mediaUrl);
                        viewHolder.video.start();
                       viewHolder.image.setVisibility(View.GONE);
                    } else {
                        viewHolder.video.setVisibility(View.GONE);
                        GlideApp.with(PostsFragment.this).load(post.mediaUrl).apply(RequestOptions.centerCropTransform()).into(viewHolder.image);
                        viewHolder.image.setVisibility(View.VISIBLE);
                    }
                ...
                viewHolder.numLikes.setText(String.valueOf(post.likes.size()+" Me gusta"));
